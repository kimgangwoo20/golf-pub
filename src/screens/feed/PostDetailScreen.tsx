...
const CURRENT_USER_ID = 1; // Mock: 현재 로그인 사용자 ID
...
export const PostDetailScreen: React.FC = () => {
  ...
  const handleSendComment = () => {
    ...
    if (replyingTo) {
      // 대댓글 추가
      Alert.alert('대댓글 작성', '대댓글 기능은 개발 예정입니다.');
      setCommentText('');
      setReplyingTo(null);
    } else {
      // 댓글 추가
  ...
  const handlePostMenu = () => {
    const isMyPost = post.author.id === CURRENT_USER_ID;
  ...
  const handleCommentMenu = (comment: Comment) => {
    const isMyComment = comment.author.id === CURRENT_USER_ID;
